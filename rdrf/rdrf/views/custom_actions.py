...
class CustomActionView(View):
    ...
    def get(self, request, action_id, patient_id):
        ...
        if requires_gui:
            ...
            cae.save()
            return self._polling_view(
                request, user, custom_action, task_id, patient_model, cae
            )
        else:
            ...
            try:
                ...
                cae.status = "error"
                cae.save()
                logger.error("Error custom action cae %s: %s" % (cae.id, ex))
                raise
    ...
    def _generate_gui(self, request, custom_action, cae):
    ...
    def post(self, request, action_id, patient_id):
        ...
        if custom_action.asynchronous:
            ...
            cae.save()
            return self._polling_view(
                request, user, custom_action, task_id, patient_model
            )
            ...
            try:
                ...
                try:
                    ...
                    cae.error_string = str(ex)
                    cae.save()
                    logger.error("Error running cae %s: %s" % (cae.id, ex))
                    raise
    ...
    def _polling_view(
        self, request, user, custom_action, task_id, patient_model=None, cae=None
    ):
...
class CustomActionWrapper:
    ...
    def url(self):
        if self.valid:
            ...
            return reverse(
                "custom_action", args=[self.custom_action.pk, self.patient_model.pk]
            )
        else:
            return ""
    ...
    def _check_validity(self):
        return self.custom_action.check_security(self.user, self.patient_model)
