            ...
            if self.cde.max_value:
                def validate_max(value):
                    ...
                    if value > self.cde.max_value:
                        raise ValidationError("Value of %s for %s is more than maximum value %s" % (value, self.cde.name, self.cde.max_value))
                validators.append(validate_max)

            if self.cde.min_value:
                def validate_min(value):
                    if value < self.cde.min_value:
                        raise ValidationError("Value of %s for %s is less than minimum value %s" % (value, self.cde.name, self.cde.min_value))
                ...
                try:
                    ...
                    def validate_pattern(value):
                        if not re_pattern.match(value):
                            raise ValidationError("Value of %s for %s does not match pattern '%s'" % (value, self.cde.name, self.cde.pattern))
            ...
            if self.cde.max_length:
                def validate_length(value):
                    if len(value) > self.cde.length:
                        raise ValidationError("Value of '%s' for %s is longer than max length of %s" % (value, self.cde.name, self.cde.max_length))
