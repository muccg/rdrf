...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--man", action="store_true", help="Help about this command."
        )
        parser.add_argument(
            "--users",
            action="store_true",
        ...
        parser.add_argument("--json", action="store_true", help="Format result in JSON")
    ...
    def handle(self, *args, **options):
        ...
        print(
            "Detecting form/section/cde codes that are in the ClinicalData data but does not exists anymore...\n"
        )
    ...
    def display_help(self):
        ...
        print("-----------------------------\n")
        print(
            "This admin command displays the form names, section codes and cde codes that are causing RDRF to crash.\n"
        )
        print("\n-----------------------------\n")
        print("WHY DOES THE PROBLEM OCCUR?\n")
        print("-----------------------------\n")
        print(
            "This situation happens when designers edit form names and section/cde codes. Our code logic does not erase or update the ClinicalData under these names and codes, causing the site to crash.\n"
        )
        print("\n-----------------------------\n")
        print("WHAT SHOULD YOU DO?\n")
        print("-----------------------------\n")
        print(
            "Till we update the edit code logic, you must create a script updating or removing these obsolete ClinicalData data.\n"
        )
    ...
    def get_bad_codes(self):
        ...
        if self.display_users:
            self.display_user_row(
                "PATIENT ID", "CONTEXT ID", "TYPE", "NAME/CODE", "VALUE"
            )
            self.display_user_row(
                "----------",
                "---------",
        ...
        for clinicaldata in clinicaldatas:
            if clinicaldata.collection == "history":
                bad_codes = self.get_bad_codes_from_collection(
                    clinicaldata.data, form_names, section_codes, cde_codes, bad_codes
                )

        return bad_codes

    def get_bad_codes_from_collection(
        self, collection_cdes_data, form_names, section_codes, cde_codes, bad_codes
    ):
        ...
        for form in collection_cdes_data["record"]["forms"]:
            if (
                form["name"] not in form_names
                and form["name"] not in bad_codes["form_names"]
            ):
                if self.display_users:
                    self.users_listing.append(
                        self.display_user_row(
                            patient_id, context_id, "form", form["name"]
                        )
                    )
                bad_codes["form_names"].append(form["name"])
            for section in form["sections"]:
                if (
                    section["code"] not in section_codes
                    and section["code"] not in bad_codes["section_codes"]
                ):
                    if self.display_users:
                        self.users_listing.append(
                            self.display_user_row(
                                patient_id, context_id, "section", section["code"]
                            )
                        )
                ...
                if section["allow_multiple"]:
                    for sub_cdes in section["cdes"]:
                        for cde in sub_cdes:
                            if (
                                cde["code"] not in cde_codes
                                and cde["code"] not in bad_codes["cde_codes"]
                            ):
                                if self.display_users:
                                    self.users_listing.append(
                                        self.display_user_row(
    ...
    def display_user_row(
        self,
        patient_id,
        context_id,
        element,
        code,
        value="RDRF_NOT_SET_display_user_row",
    ):
        ...
        if value != "RDRF_NOT_SET_display_user_row":
            self.stdout.write(
                self.style.ERROR(
                    f"{formatted_patient_id} | {formatted_context_id} | {formatted_element} | {formatted_code} | {value}"
                )
            )
            return {
            ...
            self.stdout.write(
                self.style.ERROR(
                    f"{formatted_patient_id} | {formatted_context_id} | {formatted_element} | {formatted_code} |"
                )
            )
            return {
                "patient_id": patient_id,
