...
def backup_patient_consent_files(patient, isBackedup=False):
    consents = PatientConsent.objects.filter(patient=patient.id)
    if consents is None:
        print("No Patient consent found")
        return isBackedup
    for consent in consents:
        if consent.form is '' or consent.form is None:
            print("consent form is empty")
            break
        consent_file = settings.MEDIA_ROOT + "/" + str(consent.form)
        if os.path.isfile(consent_file):
            dir_name = "patient_id_" + str(patient.id) + "/consents/consentfile_id_" + str(consent.id)
            backup_patient_consents_dir = create_backup_dir(dir_name)
            print("CONSENT file: %s" % consent_file)
            print("Backup directory %s" % backup_patient_consents_dir)
            copy_file(consent_file, backup_patient_consents_dir)
            isBackedup = True
        else:
            print("Consent file '%s' is not found" % consent_file)
    return isBackedup


def backup_clinicaldata_cdefiles(patient, isBackedup=False):
            ...
            for form_dict, section_dict, cdefile_dict in cdefiles_generator:
                ...
                if cdefile_id:
                    ...
                    if os.path.isfile(cde_file):
                        ...
                        print("CDEFile '%s': '%s' is not found" % (cdefile_dict['code'], cde_file))
                else:
                    print("CDEFile '%s' is empty: %s" % (cdefile_dict['code'], cdefile_dict))
    return isBackedup


def backup_reg_specific_files(patient, isBackedup=False):
    registry_model = patient.rdrf_registry.first()
    ...
    if cds is None:
        print("No Registry specific data found")
        return isBackedup
    for cd in cds:
        ...
        for key, value in registry_data.items():
            ...
            if is_file(reg_spec_cde_dict):
                ...
                if cdefile_id:
                    cdefile_item, cdefile_filename = get_file(cdefile_id)
                    reg_spec_cde_file = settings.MEDIA_ROOT + "/" + str(cdefile_item)
                    if os.path.isfile(reg_spec_cde_file):
                        ...
                        print("Registry Specific file: %s" % reg_spec_cde_file)
                        print("Backup directory %s" % backup_reg_spec_file_dir)
                        copy_file(reg_spec_cde_file, backup_reg_spec_file_dir)
                        isBackedup = True
                    else:
                        print("'%s': '%s' is not found" % (reg_spec_cde_dict['code'], reg_spec_cde_file))
                else:
                    print("'%s' is empty: %s" % (reg_spec_cde_dict['code'], reg_spec_cde_dict))
    return isBackedup
...
if __name__ == "__main__":
    ...
    for patient in patients:
        ...
        is_backup = backup_patient_consent_files(patient)
        if is_backup is False:
            print("No data to perform backup from filesystem")
        print("......Backing up ClinicalData CDEFile......")
        is_backup = backup_clinicaldata_cdefiles(patient)
        if is_backup is False:
            print("No data to perform backup from filesystem")
        print("......Backing up Registry Specific File......")
        is_backup = backup_reg_specific_files(patient)
        if is_backup is False:
            print("No data to perform backup from filesystem")
        print(" ***************************************************************** ")
