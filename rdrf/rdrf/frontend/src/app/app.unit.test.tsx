...
function sel(h: HTMLElement): HTMLInputElement {
  return h as HTMLInputElement;
}

function btn(h: HTMLElement) : HTMLButtonElement {
...
describe("Component tests: A test App using Redux", () => {
  ...
  describe("Basic tests", () => {
      ...
      */
      it("is on the first question", () => {
        ...
        const previousBtn = screen.getByText("Previous") as HTMLButtonElement | null;
      ...
      */
      it('can go to the next question from the first question', () => {
        ...
        const nextButton = btn(screen.getByText("Next"));
  ...
  describe("Preconditional tests: 'or'", () => {
      ...
      */
      it('only has three questions', () => {
        ...
        const previousBtn = screen.getByText("Previous") as HTMLButtonElement | null;
        expect(previousBtn.disabled).toEqual(true);
        const nextBtn = screen.getByText("Next") as HTMLButtonElement | null;
        ...
        expect(previousBtn.disabled).toEqual(false);
        expect(nextBtn.disabled).toEqual(false);
      ...
      */
      it('can have data entered', () => {
        ...
        const answer1Sel = answer1Select as HTMLInputElement;

        expect(answer1Sel.checked).toEqual(false);
        ...
        expect(answer1Sel.checked).toEqual(true);
      ...
      */
      it('has the third question (preconditional) available after entering data in the first question', () => {
        ...
        const nextButton = btn(screen.getByText("Next"));
        expect((nextButton as HTMLButtonElement).disabled).toEqual(false);
        expect(testStore.getState().stage).toEqual(0);
      ...
      */
      it('has the third question remain available after changing data in the first question', () => {
        ...
        const nextButton = btn(screen.getByText("Next"));
        const prevButton = btn(screen.getByText("Previous"));
  ...
  describe("Preconditional tests: '='", () => {
      ...
      */
      it('does not make the fourth question available if "Good" is answered in the third question', () => {
        ...
        goodAnswerSelect = sel(screen.getByLabelText("Good"));
        notGoodSelect = sel(screen.getByLabelText("Not so good"));
      ...
      */
      it('does not make the fourth question available if "Not so good" is answered in the third question', () => {
        ...
        const nextButton = btn(screen.getByText("Next"));
        const prevButton = btn(screen.getByText("Previous"));
        let goodAnswerSelect =sel(screen.getByLabelText("Good"));
        let notGoodSelect = sel(screen.getByLabelText("Not so good"));
        let badAnswerSelect = sel(screen.getByLabelText("Bad"));
  ...
  describe("Preconditional tests: 'contains'", () => {
      ...
      */
      it('does not make the fifth question available if "Work is stressful" is selected in the fourth question', () => {
        ...
        const nextButton = btn(screen.getByText("Next"));
        const prevButton = btn(screen.getByText("Previous"));
        let workSelect = sel(sel(screen.getByLabelText("Work is stressful")));
        let friendsSelect = sel(sel(screen.getByLabelText("Can't see my friends")));
        let reactSelect = sel(sel(screen.getByLabelText("React is difficult")));
        let daySelect = sel(sel(screen.getByLabelText("Just a bad day")));
        ...
        workSelect = sel(screen.getByLabelText("Work is stressful"));
        friendsSelect = sel(screen.getByLabelText("Can't see my friends"));
        reactSelect = sel(screen.getByLabelText("React is difficult"));
        daySelect = sel(screen.getByLabelText("Just a bad day"));
      ...
      */
      it('does not make the fifth question available if "Can\'t see my friends" and "Just a bad day" are selected in the fourth question', () => {
        ...
        const nextButton = btn(screen.getByText("Next"));
        const prevButton = btn(screen.getByText("Previous"));
        let workSelect = sel(screen.getByLabelText("Work is stressful"));
        let friendsSelect = sel(screen.getByLabelText("Can't see my friends"));
        let reactSelect = sel(screen.getByLabelText("React is difficult"));
        let daySelect = sel(screen.getByLabelText("Just a bad day"));
  ...
  describe("Preconditional tests: 'intersection'", () => {
      it('does not make the sixth question available if "It\'s hard to read" is selected in the fifth question', () => {
        ...
        const nextButton = btn(screen.getByText("Next"));
        const prevButton = btn(screen.getByText("Previous"));
        let hardToReadSelect = sel(screen.getByLabelText("It's hard to read"));
        let dependenciesSelect = sel(screen.getByLabelText("It has too many dependencies"));
        let facebookSelect = sel(screen.getByLabelText("It's made by Facebook"));
        let noUnderstandingSelect = sel(screen.getByLabelText("I don't understand it"));
        ...
        hardToReadSelect = sel(screen.getByLabelText("It's hard to read"));
        dependenciesSelect = sel(screen.getByLabelText("It has too many dependencies"));
        facebookSelect = sel(screen.getByLabelText("It's made by Facebook"));
        noUnderstandingSelect = sel(screen.getByLabelText("I don't understand it"));
  ...
  describe('Datatype tests: date', () => {
    ...
    it('displays a date box in Question 10', () => {
      ...
      let dateBox: HTMLInputElement;

      expect(() => {
        // dateBox = screen.getByRole("datetime");
        dateBox = screen.getByDisplayValue("") as HTMLInputElement;
      }).not.toThrow();
  ...
  describe('Datatype tests: range', () => {
    it('displays 3 unchecked radio select buttons in Question 12', () => {
      ...
      radioButtons.forEach((rButton : HTMLInputElement) => {
  ...
  describe('Datatype tests: multiselect', () => {
    it('displays four unchecked multiselect options in Question 13', () => {
      ...
      checkboxes.forEach((cBox: HTMLInputElement) => {
...
describe('Navigational tests: the app', () => {
  ...
  it('retains checked radio select answers when returning to question', () => {
    ...
    const nextButton = btn(screen.getByText("Next"));
    const prevButton = btn(screen.getByText("Previous"));
    let answer1Select = sel(screen.getByLabelText("Answer 1", { exact: false }));
    let answer2Select = sel(screen.getByLabelText("Answer 2", { exact: false }));
