...
class HL7Handler:
    ...
    def _create_message_model(self, registry_code, message):
        ...
        try:
            ...
            logger.error(f"Error getting event_code:{ex}")
            event_code = "error"
        try:
            umrn = get_umrn(message)
        except Exception as ex:
            logger.error(f"Error getting umrn:{ex}")
            umrn = ""
        ...
        logger.info("saving message model ...")
        try:
            message_model.save()
            logger.info(f"saved message model: {message_model.id}")
            return message_model
        except Exception as ex:
            logger.error(f"Could not save message model: {ex}")
    ...
    def handle(self) -> Optional[dict]:
        ...
        try:
            ...
            if field_dict is None:
                ...
                if message_model is not None:
                    message_model.error_message = "field_dict is None"
                    message_model.save()
                return {"error": "field_dict is None"}

            logger.info("parsing demographics fields ...")
            self.patient_attributes = self._parse_demographics_fields(field_dict)
            logger.info("parsing cde fields ...")
            self.patient_cdes = self._parse_cde_fields(field_dict)
            umrn = self.patient_attributes.get("umrn", "")
            ...
            if message_model is not None:
                message_model.error_message = f"Runtime error: {ex}"
                message_model.save()
