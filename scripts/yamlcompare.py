...
def get_cde_usage(code, forms):
    for form in forms:
        sections = form["sections"]
        for section in sections:
            if code in section["elements"]:
                return "%s/%s/%s" % (form["name"], section["code"], code)


def compare_dicts(category, dict1, dict2, result_file, cde_dict=None, survey_name=None, forms=None):
...
with open(yaml_file_1) as yf1, open(yaml_file_2) as yf2, open(out_file, 'w+') as result_file:
    ...
    forms = data1["forms"]
    result_file.write("Category,Item,Attribute,ValueA,ValueB\n")

    # compare CDEs
    cde_dict1 = get_dict_of_items(data1, "cdes", "code")
    cde_dict2 = get_dict_of_items(data2, "cdes", "code")
    compare_dicts("CDE", cde_dict1, cde_dict2, result_file, forms=forms)
